    ...
    >
      <div class="form">
        ...
        >
          ...
          <el-form-item label="封面" prop="img_url" v-model="formData.img_url">
            <el-upload
  ...
  data() {
    return {
      ...
      form_rules: {
        ...
        img_url: [{ required: true, message: "不能为空", trigger: "blur" }]
      }
    };
  },
  methods: {
    onSubmit(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          ...
          const url = this.DialogMenu.option == 'add' ? 'add' :`edit/${this.formData.id}`;
          //option用来区分是点的编辑还是添加，以此来决定调用哪个接口
          this.$axios.post(`http://www.chenruisheng.info/aoaoe/api/v1.0/admin/menu/${url}`,this.formData).then((res) => {
            this.$message({
              message:"数据执行完毕",
              type:"success"
            })
    ...
    handleAvatarSuccess(res, file) {
      // console.log(res)
        ...
        this.formData.img_url = this.imageUrl
      },
